...
export default function CountryCard({name, flags, population, region, capital}) {
    return (
      <article className="bg-skin-card rounded-md h-full w-full shadow-md">
        <img alt={`${name.common}'s flag`} src={flags.svg} className="block h-44 w-full rounded-t-lg object-cover"/>
        <div className="p-6">
          <Link href={`./${name.common}`}>
            <p className="text-skin-base text-lg font-semibold"> {name.common} </p>
          </Link>
